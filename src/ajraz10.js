...
function closePopup(remember = true) {
  ...
  if (remember && !previewEnabled()) localStorage.setItem(STORAGE_DISMISSED, String(Date.now()));
}
...
function showPopup() {
  ...
  if (!previewEnabled() && dismissed && Date.now() - dismissed < DISMISS_COOLDOWN_MS) return;
  ...
  overlay.innerHTML = `
    <section class="ajraz10-modal" role="dialog" aria-modal="true" aria-labelledby="ajraz10-title">
      ...
      <div class="ajraz10-code-mark">AJRAZ10</div>
      ...
      <form id="ajraz10-form">
        ...
        <label><span>Nombre <small>(opcional)</small></span><input name="name" type="text" autocomplete="name" maxlength="80"></label>
        <label><span>Correo electrónico</span><input name="email" type="email" autocomplete="email" required></label>
        <label class="ajraz10-consent"><input name="consent" type="checkbox" required><span>Acepto recibir comunicaciones del Archivo 066 y esta promoción por correo.</span></label>
  ...
  overlay.querySelector(".ajraz10-close").addEventListener("click", () => closePopup(true));
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result?.error || "No pudimos habilitar el acceso.");
      localStorage.setItem(STORAGE_SUBSCRIBED, result.email || String(data.get("email") || ""));
      ...
      overlay.querySelector(".ajraz10-close").addEventListener("click", () => closePopup(false));
      overlay.querySelector(".ajraz10-go-buy").addEventListener("click", () => {
        closePopup(false);
        document.getElementById("compra-directa")?.scrollIntoView({ behavior: "smooth", block: "start" });
      });
    } catch (error) {
...
function getCheckoutEmailInput() {
  return document.querySelector("#compra-directa .sale-form input[type='email']");
}
...
function injectCheckoutPromo() {
  ...
  block.innerHTML = `
    ...
    <div class="ajraz10-checkout-row"><input id="ajraz10-code-input" type="text" maxlength="24" autocomplete="off" placeholder="Ej: AJRAZ10" aria-label="Código de acceso"><button id="ajraz10-apply" type="button">APLICAR</button></div>
  ...
  codeInput.addEventListener("input", () => {
    ...
    appliedPromo = null;
    status.className = "";
    status.textContent = "Los códigos privados se validan con el correo utilizado en la compra.";
    restoreSummary();
  });
  emailInput?.addEventListener("input", () => {
    if (appliedPromo && emailInput.value.trim().toLowerCase() !== appliedPromo.email) {
      appliedPromo = null;
      status.className = "is-error";
      status.textContent = "El correo cambió. Vuelve a aplicar AJRAZ10.";
      restoreSummary();
    }
  });
  apply.addEventListener("click", async () => {
    ...
    if (!email) {
      status.className = "is-error";
      status.textContent = "Ingresa primero el correo que usarás para comprar.";
      return;
    }
    apply.disabled = true;
    status.className = "";
    ...
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result.ok) throw new Error(result?.error || "Código no autorizado.");
      appliedPromo = { code: result.code, email, discountAmount: result.discountAmount, discountedBookPrice: result.discountedBookPrice };
      status.className = "is-success";
      status.textContent = "✓ ACCESO AJRAZ10 AUTORIZADO · 10% aplicado al precio del libro.";
      applyPromoToSummary();
    } catch (error) {
      appliedPromo = null;
      status.className = "is-error";
...
function restoreSummary() {
  ...
  if (rows[0]?.querySelector("strong")) rows[0].querySelector("strong").textContent = formatCLP(ORIGINAL_BOOK_PRICE);
  const region = getCheckoutRegionSelect()?.value || "RM";
  const shipping = region === "RM" ? 3000 : 4500;
  if (rows[2]?.querySelector("strong")) rows[2].querySelector("strong").textContent = formatCLP(ORIGINAL_BOOK_PRICE + shipping);
}
...
function applyPromoToSummary() {
  ...
  const region = getCheckoutRegionSelect()?.value || "RM";
  const shipping = region === "RM" ? 3000 : 4500;
  if (rows[0]?.querySelector("strong")) rows[0].querySelector("strong").textContent = formatCLP(DISCOUNTED_BOOK_PRICE);
  let discountRow = summary.querySelector(".ajraz10-summary-discount");
  if (!discountRow) {
    discountRow = document.createElement("div");
    discountRow.className = "ajraz10-summary-discount";
    discountRow.innerHTML = `<span>Descuento AJRAZ10</span><strong>−${formatCLP(DISCOUNT_AMOUNT)}</strong>`;
  ...
  if (total) total.textContent = formatCLP(DISCOUNTED_BOOK_PRICE + shipping);
}
...
function installFetchInterceptor() {
  ...
  window.fetch = async function ajraz10Fetch(input, init = {}) {
    ...
    const isCheckout = rawUrl === "/api/create-preference" || rawUrl.endsWith("/api/create-preference");
    if (!isCheckout) return originalFetch(input, init);
    ...
    body.promoCode = code;
    return originalFetch("/api/create-promo-preference", { ...init, body: JSON.stringify(body) });
  };
}
...
function maintainPromoUI() {
  const observer = new MutationObserver(() => {
    injectCheckoutPromo();
    if (appliedPromo) requestAnimationFrame(applyPromoToSummary);
  });
  observer.observe(document.documentElement, { subtree: true, childList: true, characterData: true });
  injectCheckoutPromo();
  document.addEventListener("change", (event) => {
    if (event.target === getCheckoutRegionSelect() && appliedPromo) requestAnimationFrame(applyPromoToSummary);
  });
}
...
if (typeof window !== "undefined") {
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => { maintainPromoUI(); schedulePopupAfterIntro(); }, { once: true });
  } else {
    maintainPromoUI();
